...
export const createChatRoom = async (req, res) => {
  try {
    ...
    participants.forEach((participantId) => {
      const socketId = io.onlineUsers.get(participantId);
      if (socketId) {
        io.to(socketId).emit("newChatRoom", { chat });
      }
    });
...
export const deleteMessage = async (req, res) => {
  try {
    ...
    if (deleteForEveryone) {
      ...
      io.to(chatId).emit("messageDeletedForUser", { messageId, userId });
...
export const addAdmin = async (req, res) => {
  try {
    ...
    io.to(chatId).emit("adminAdded", { userId });
...
export const removeAdmin = async (req, res) => {
  try {
    ...
    io.to(chatId).emit("adminRemoved", { userId });
...
export const addParticipant = async (req, res) => {
  try {
    ...
    io.to(chatId).emit("participantAdded", { userId });
...
export const removeParticipant = async (req, res) => {
  try {
    ...
    io.to(chatId).emit("participantRemoved", { userId });
